package cn.wuqinxuan.algorithm;

import cn.wuqinxuan.main.Debug;
    ...
    public static String calculate(String expression) throws ExpressionIllegalException {
        ...
        while (index < units.size() || operators.size() != 0) {
            ...
            if (unit.getType() == Unit.Type.OPERAND) {
                ...
                if (operators.empty() || OperatorComparator.compareOperator(operators.peek(), unit) == '<') {
                    operators.push(unit);
                    Debug.debug(unit + "push");
                    index += 1;
                } else if (OperatorComparator.compareOperator(operators.peek(), unit) == '=') {
